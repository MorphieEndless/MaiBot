...
# 路径前缀由父路由 /api/chat 统一提供
router = APIRouter(tags=["ChatStreams"])
...
def _target_config_to_dict(config_item: Any) -> Optional[Dict[str, Any]]:
    ...
    platform, item_id, rule_type = ChatConfigUtils.target_values(config_item)
    return {
        "platform": platform,
        "item_id": item_id,
        "type": rule_type,
...
def _talk_rule_to_dict(rule: Any, session_id: str, is_group_chat: bool, now_min: int) -> Optional[Dict[str, Any]]:
    ...
    rule_time = ChatConfigUtils.get_rule_time(rule)
    time_priority = ChatConfigUtils.talk_rule_time_priority(rule_time, now_min)
    value = ChatConfigUtils.get_rule_value(rule)
    return {
        "platform": platform,
        "item_id": item_id,
        "type": rule_type,
...
def _talk_rule_to_config_dict(rule: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        "time": ChatConfigUtils.get_rule_time(rule),
        "value": ChatConfigUtils.get_rule_value(rule),
    }
...
def _chat_session_detail_to_response(chat_session: ChatSession) -> Dict[str, Any]:
    ...
    return {
        ...
        "expression": {
            ...
            "matched_rule": _target_config_to_dict(ExpressionConfigUtils.find_expression_config_item(session_id)),
        },
        "behavior": {
            "use": behavior_use,
            "learn": behavior_learn,
            "matched_rule": _target_config_to_dict(BehaviorConfigUtils.find_behavior_config_item(session_id)),
        },
        "jargon": {
            "use": jargon_use,
            "learn": jargon_learn,
            "matched_rule": _target_config_to_dict(JargonConfigUtils.find_jargon_config_item(session_id)),
        },
